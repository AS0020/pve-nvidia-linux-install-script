...
gpu_consumer_unit_is_safe() {
    ...
    [[ "$unit" =~ ^(jellyfin|plexmediaserver|emby-server|fileflows|tdarr|frigate|nvidia-persistenced)(@[A-Za-z0-9_.:-]+)?\.service$ ]]
}
...
# Erstellt durch nvidia-driver-setup-v2.13.0.sh
blacklist nouveau
    ...
    while true; do
        ...
        if ((INTERRUPTED_TRANSACTION_PENDING)) \
           && [[ "$operation" != "check" && "$operation" != "dry-run" && "$operation" != "update-preview" ]]; then
            warn "Wegen der unterbrochenen Transaktion sind nur Diagnose und Dry-Run zulässig. Wähle beim nächsten Start Fortsetzen oder Rollback."
            continue
        fi
...
remove_forbidden_lxc_packages_before_repair() {
    ...
    ((${#forbidden[@]})) || return 0
    warn "Entferne unzulässige LXC-Kernel-/DKMS-/Treiber-/Host-Hilfspakete vor jeder dpkg-Konfiguration: ${forbidden[*]}"
    ...
    record_repair_action "Unzulässige LXC-Kernel-/DKMS-/Treiber-/Host-Hilfspakete vor dpkg-Konfiguration entfernt"
}

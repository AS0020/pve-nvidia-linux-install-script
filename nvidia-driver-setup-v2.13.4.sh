...
official_target_package_version() {
    # Mehrere offizielle Revisionen desselben Treibers sind normal (z. B.
    # 615.71.09-1 und -2). Wie beim Update die höchste vollständige DEB-Version
    # wählen; Epoch, exakte Distribution und kollidierende Fremdquellen prüfen.
...
# Erstellt durch nvidia-driver-setup-v2.13.4.sh
blacklist nouveau
    ...
    if [[ "$effective_mode" == "host" ]]; then
        ...
        if command -v mokutil >/dev/null 2>&1; then
            ...
            kernel_security_log="$(journalctl -k -b --no-pager 2>/dev/null \
                | grep -iE 'nvidia.*(key was rejected|required key not available)' \
                | tail -n3 || true)"
            # "module verification failed ... tainting kernel" beschreibt
            # einen erlaubten Ladevorgang im permissiven Modus, keine Ablehnung.
            # Echte Schlüsselablehnungen bleiben auch bei noch geladenem altem
            # Modul kritisch, da sie dessen neue Ersatzversion betreffen können.
...
summarize_nvidia_lxc_containers() {
    ...
    printf '  %-7s %-18s %-9s %-8s %-24s %-12s %s\n' CTID NAME STATUS ERGEBNIS USERSPACE-DEBIAN NVML HOSTPAKETE
...
build_nvidia_update_plan() {
    ...
    while IFS=$'\t' read -r package current status; do
        ...
        if [[ "$MODE" == lxc && "$package" =~ $LXC_FORBIDDEN_PACKAGE_REGEX ]]; then
            if ((AUTOMATIC_REPAIR)) && [[ "${package%%:*}" == nvidia-modprobe ]]; then
                # Altbestand aus früheren LXC-Profilen: nur diesen Helfer im
                # gemeinsamen Updateplan ablösen, niemals Kernel/DKMS ausführen.
                # Simulation, Rückkehrbasis und Fremdpaketschutz bleiben aktiv.
                append_unique NVIDIA_UPDATE_REMOVALS "$package"
                changes+=("$package"$'\t'"$current"$'\t'"entfernen (LXC-Hosthelfer)"$'\t'"-")
                log "LXC-Update plant die sichere Entfernung des alten Hosthelfers $package."
                continue
            fi
...
for package in "${PREFLIGHT_CANDIDATE_PACKAGES[@]}"; do
    ...
    # Ein alter APT-Pin darf den bereits simulierten exakten Zielstand nicht
    # wieder durch eine ältere Revision ersetzen. Policy bleibt Diagnose;
    # installiert wird explizit die geprüfte offizielle vollständige Version.
    candidate="$(official_target_package_version "$package" || true)"
    [[ -n "$candidate" ]] \
        || die "Keine eindeutig offizielle Zielversion für $package=$TARGET_VERSION nach dem Pinning verfügbar."
    debian_versions_equal "$candidate" "$PREPIN_COMMON_DRIVER_DEBIAN_VERSION" \
        || die "Die offizielle Zielrevision hat sich seit der Vorabprüfung verändert: $package=$candidate, zuvor $PREPIN_COMMON_DRIVER_DEBIAN_VERSION. Keine Treiberinstallation auf Basis eines geänderten Plans."
